...
비전 모듈이 3종의 마감재 이미지를 전수 처리하여 014 오케스트레이터로 전달하고, 오케스트레이터가 가공성 레벨과 SFE 물리 보정을 적용한 최종 값은 다음과 같습니다.

| 재료 표면 분류 | 측정 SFE (mN/m) | 표면 조도 Ra (um) | 물리 보정 적용 SFE (mN/m) | 일반적인 SFE (mN/m) | 최종 가공 가혹도 등급 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **2B Finish** | 38.6 | 1.15 | **38.6** | 39.0 | **Level 1** |
| **BA Finish** | 42.3 | 0.82 | **42.3** | 42.5 | **Level 2** |
| **PCM Hairline (HL)** | 34.1 | 0.28 | **37.4** | 37.5 | **Level 3** |

*물리 보정 상세:* 헤어라인(HL) 마감에 의한 접촉선 고정(Contact Line Pinning) 및 공기 갇힘 현상(Cassie-Baxter 상태)으로 인해 겉보기 표면에너지(Apparent SFE)가 34.1로 왜곡 계측되었으나, 오케스트레이터가 조도(0.28 um)를 기반으로 물리 보정 수식 `SFE * (1 + 0.35 * Ra)`을 적용하여 실제 참값인 37.4 mN/m로 복원 성공했습니다. 2B와 BA는 오차 범위 내에 해당하여 보정 전후 수치가 일치합니다.
