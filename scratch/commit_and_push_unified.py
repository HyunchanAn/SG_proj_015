...
if __name__ == "__main__":
    for repo in repos_to_commit:
        ...
        # Git commit
        ok, commit_out = run_git(repo, ["commit", "-m", "docs: standardize development log formatting and clean duplicates"])
        if not ok:
        ...
        # Git pull --rebase
        print(f"Pulling remote changes for {branch_name}...")
        ok, pull_out = run_git(repo, ["pull", "--rebase", "origin", branch_name])
        if not ok:
            print(f"Failed git pull: {pull_out}")
            continue
        print(pull_out.strip())
